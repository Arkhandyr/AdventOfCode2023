...
func GetGamePower(game string) int {
	...
	for _, round := range rounds {
		...
		SplitRounds(colors, &gameMax)
		gameTotal = gameMax[0] * gameMax[1] * gameMax[2]
	}

	return gameTotal
}

func SplitRounds(colors []string, gameMax *[]int) {
	for _, color := range colors {
		CompareColorValues(color, gameMax)
	}
}

func CompareColorValues(color string, gameMax *[]int) {
...
func GetValue(color string) int {
