...
func main() {
	...
	for scanner.Scan() {
		...
		if ValidGame(game) {
			total += gameNumber
			fmt.Print(strconv.Itoa(gameNumber) + " ")
		}
	}
...
func ValidGame(game string) bool {
	rounds := strings.Split(game, ";") //separa rodadas do jogo
	valid := true
	for _, round := range rounds {
		colors := strings.Split(round, ",") //separa cores das rodadas

		if !ValidateRound(colors) {
			valid = false
			break
		}
	}
...
func ValidateRound(colors []string) bool {
	valid := true
...
func ValidateColor(color string) bool {
	...
	if strings.Contains(color, "red") {
		valid = ValidateColorValue(color, red)
	} else if strings.Contains(color, "green") {
		valid = ValidateColorValue(color, green)
	} else if strings.Contains(color, "blue") {
		valid = ValidateColorValue(color, blue)
	}

	return valid
}

func ValidateColorValue(color string, limit int) bool {
