...
func main() {
	...
	var currentNodes = FindStarterNodes()
	...
	for !AllNodesEndWithZ(currentNodes) {
		for _, direction := range loop {
			for i, currentNode := range currentNodes {
				currentNodes[i] = NextNode(currentNode, direction)
			}
			fmt.Println(currentNodes)
			steps++
		}
...
func FindStarterNodes() []string {
	var currentNodes = []string{}
	for _, node := range nodes {
		if node.Origin[2] == 'A' {
			currentNodes = append(currentNodes, node.Origin)
		}
	}
